...
void uscalec_fixed(char *dst, double f, int n)
{
    fixed(dst, f, n);
}

void uscalec_short(char *dst, double f)
{
    short1(dst, f);
}

double uscalec_parse(const char *s, size_t len)
{
    return uscalecStrtod(s, s + len);
}
